...
public class HuffmanCoding2 {
     ...
     */
    public HuffmanCoding2(String f) { 
        fileName = f; 
    }
    ...
    public void makeTree() {
    	...
    	while (source.size() > 0 || target.size() > 1) {
			...
			CharFreq cn = new CharFreq(null,t1.getData().getProbOcc()+t2.getData().getProbOcc());
			TreeNode nn = new TreeNode(cn,t1, t2);
			target.enqueue(nn);    		
    	}
    	huffmanRoot=target.dequeue();
    }

    private void traverseTree(TreeNode node, String st) {
    	if (node == null)
    		return;
    	if (node.getLeft() != null) {
    		...
    		if (left.getData().getCharacter() != null)
    			encodings[left.getData().getCharacter()] = leftStr;
    		traverseTree(left,leftStr);
    	}
    	if (node.getRight() != null) {
    		String rightStr = st+"1";
    		TreeNode right = node.getRight();
    		if ( right.getData().getCharacter() != null)
    			encodings[right.getData().getCharacter()] = rightStr;
    		traverseTree(right,rightStr);
    	}
    }
    ...
    public void encode(String encodedFile) {
        ...
        while (StdIn.hasNextChar()) {
        	...
        	String enc = encodings[ch];
        	buf.append(enc);
        }
        writeBitString(encodedFile,buf.toString());
    }
    ...
    public void decode(String encodedFile, String decodedFile) {
        ...
        String inStr = readBitString(encodedFile);
        ...
        for (int i=0; i < inStr.length(); i++) {
        	...
        	if (node.getLeft()==null || node.getRight()==null) {
    			out.append(node.getData().getCharacter());
    			node=huffmanRoot;
        	}        	        	
        }
